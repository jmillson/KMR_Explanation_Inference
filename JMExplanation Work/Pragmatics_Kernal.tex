...
%Derivative concepts: \textit{common score}=$ \cap ${Speaker's deontic scores}? 
...
\fntext{ContrastPW}{The inclusion of incoherent sets is just one of the many reasons why we should not identify commitment positions with possible worlds. According to what Lewis calls \textit{linguistic ersatzism} possible worlds identified with maximally consistent sets of sentences. As elements of $\mathcal{P}(\mathcal{L}_e) $, commitment positions are neither necessarily maximal nor necessarily coherent.}

Using the concepts of commitment position and entitlement space, we can derive a few other useful notions. Let $ \alpha_i $ stand for the commitment position of an agent, \textit{i} and $ \epsilon_i $ stand for her entitlement space. The set of sentences to which \textit{i} is both committed and entitled is $ \alpha_i \cap \epsilon_i$. We call this set an agent's \textit{deontic position} and refer to her total set of commitments and entitlements, i.e. $ \alpha_i \cup \epsilon_i $, as her \textit{deontic score.}

When a speaker asserts a sentence she undertakes a commitment to demonstrate entitlement to that sentence if challenged and to refrain from asserting sentences incompatible with that sentence. Undertaking assertional commitments thus (partly) determines the shape of a speaker's entitlement space.


%When a speaker asserts a sentence she makes a certain sort of move in the game. If she asserts $ A $, then she undertakes a commitment that, \textit{inter alia}, prohibits her from asserting those sentences that are incompatible with $ A $. In this sense, asserting $ A $ has the effect of circumscribing the range of future commitments that a speaker is permitted to undertake---i.e. there are positions that she is no longer permitted to occupy. Only those sets of sentences with which $A$ is compatible are considered `live options'. (Of course, a speaker may very well retract an assertion and thereby make available those previously impermissible commitments). In this sense, we can think of an assertional commitment to $A$ as carving out a portion of the game board consisting of those positions that are now \textit{accessible} to the speaker. Since it is within this part of the game board that a speaker's \textit{entitled} commitments are to be found, we call it a speaker's \textit{entitlement space}. It is important to remember that a speaker's entitlement space not contain only those sets of possible commitments to which she \textit{is} entitled. Rather, this space represents all those sets of commitments to which she \textit{may} be entitled. Since every assertion a speaker makes alters her entitlement space, and since her total set of commitments is her commitment position, we can say that a speaker's commitment position determines her entitlement space.
...
\jm{Here is what a why-question, \textit{Why B?}, i.e. $ ?\{\Sigma\ee B, \Theta\ee B\} $ does to the game board:}

\begin{center}
\renewcommand{\arraystretch}{4}
\begin{tabular}{|c|c|}
\hline 
 $ \{\Delta : \Sigma\ee B, \Delta \not\nmc \bigperp\} \,\,\bigcap\,\, \{\Delta : \Theta\ee B, \Delta \not\nmc \bigperp\} $ & $ \{\Delta : \Sigma\ee B, \Delta \nmc \bigperp\}  \,\,\bigcap\,\, \{\Delta : \Theta\ee B, \Delta \not\nmc \bigperp\} $  \\ 
\hline 
$ \{\Delta : \Sigma\ee B, \Delta \not\nmc \bigperp\} \,\,\bigcap\,\, \{\Delta : \Theta\ee B, \Delta \nmc \bigperp\} $ & $ \{\Delta : \Sigma\ee B, \Delta \nmc \bigperp\}  \,\,\bigcap\,\, \{\Delta: \Theta\ee B, \Delta \nmc \bigperp\} $  \\ 
\hline 
\end{tabular} 
...
\jm{Can't these sets be reduced to sets of modally robust inferences or even sturdy inferences? Fuck Yeah!!!!}

\begin{itemize}
\item $ \{\Delta : \Sigma\ee B, \Delta \not\nmc \bigperp\} = W  $ where $ \Sigma\mrc{W} B $. All those sets compatible with the assertion of $ \Sigma\ee B $ are just those sets that do not defeat $ \Sigma\nmc B $!!!! 

\item Likewise: $ \{\Delta : \Sigma\ee B, \Delta \nmc \bigperp\} = \overline{W} $ where $ \Sigma\mrc{W} B $. 

\item So is this the same as the picture directly above? (note: $ \Theta\mrc{W'} B $)
\end{itemize}
...
%Now, consider the set of all the possible sets of commitments a speaker of a particular language can undertake. Depending on the sentence, these commitments may be assertional or practical (i.e. commitments to perform some action).
