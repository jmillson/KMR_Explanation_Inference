...
\begin{description}
	%\item[\underline{Axiom 1}] $ {\Gamma, A \nc}^{[\uparrow, \Uparrow,\twoheaduparrow]} B  \Rightarrow {\Gamma, A \nce}^{[\uparrow, \Uparrow,\twoheaduparrow]} B  $
	\item[\underline{Axiom 1}] If $ \Gamma, A \nc B  $ then $ \Gamma, A \nce B  $ is an axiom.
	%\item[\underline{Axiom 2}] If $ {\Gamma, A \nc}^{\uparrow W} B  $ then $ {\Gamma, A \nce}^{\uparrow W} B  $ is an axiom.
	%\item[\underline{Axiom 3}] If $ {\Gamma, A \nc}^{\Uparrow W}B  $ then ${\Gamma, A \nce}^{\Uparrow W}B  $ is an axiom.
	\item[\underline{Axiom 2}] If $ {\Gamma, A \nc}^{\twoheaduparrow} B  $ then $ {\Gamma, A \nce}^{\twoheaduparrow} B $ is an axiom.
\end{description}
...
As meta-theoretical constructions, these rules determine the rules of inference expressible in the metalanguage. They are, in that sense, meta-rules governing inferential commitments. BER says that if an agent in context $\Gamma$ is entitled to $ B $ as the GSRC of her commitment to $A$, then she is entitled to $ A \rhd B $ in the absence of a commitment to $A$. BES is a simplification rule that gives the inverse of BER. Notice that in BER, the consequence relation in the conclusion-sequent is subjunctively weaker than it is in the premise, while the opposite is true of BES. 


%The rules BER and BEL give the right and left rules for the connective $\rhd$, respectively. Right rules in sequent calculus correspond to introduction rules in natural deduction and left rules to elimination rules. BER and BEL determine the total set of inferences in $\mathcal{L}_e$ in which $ \rhd $ occurs, and so, according to inferentialism, they settle the meaning of $ \rhd $. BES is a simplifying rule, which, together with BER, entails that ${\Gamma, A \nce}^{\twoheaduparrow} B \Leftrightarrow {\Gamma \nce} A \rhd B $. Admittedly, this is not a standard deduction theorem, but it is an instance what can be called a \textit{generalized deduction theorem}. \color{red} Explain generalized deduction theorem.\color{black} 
